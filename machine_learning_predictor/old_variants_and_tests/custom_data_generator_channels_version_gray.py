...
class CustomImageDataGenerator(tf.keras.utils.Sequence):
    ...
    def __init__(self, data_frame, x_col_name, y_col_name, sequence_length, batch_size, num_classes=NUMBER_OF_CLASSES,
        ...
        self.use_grayscale = True  # TODO this generator version uses only grayscale; parameter only for compatibility
    ...
    def __getitem__(self, index):
        ...
        """
        X = np.empty((self.batch_size, self.sequence_length, self.output_size[1], self.output_size[0]))
        ...
        # Tensorflow does not support grouped convolutions and "channels_first" seems to be broken, so we need
        # to manually move the second axis (i.e. the sequence length) to the last position so they are used as channels.
    ...
    def __get_data(self, sample):
        ...
        new_sample = np.squeeze(image_sample)  # remove the last axis (i.e. the gray channel)
        return new_sample, sample_label
    ...
    def get_image_shape(self):
        return self.output_size[1], self.output_size[0], self.sequence_length
